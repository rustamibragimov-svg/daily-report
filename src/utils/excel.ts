...
async function buildWorkbook(wb: ExcelJS.Workbook, report: DailyReport) {
  const ws = wb.addWorksheet('Ежедневная отчётность');
  ws.getColumn(1).width = 4; ws.getColumn(2).width = 38; ws.getColumn(3).width = 42;
  ws.getColumn(4).width = 28; ws.getColumn(5).width = 14; ws.getColumn(6).width = 14; ws.getColumn(7).width = 14;
  let r = 1;
  ws.mergeCells(r, 1, r, 7);
  const title = ws.getCell(r, 1);
  title.value = '3PL ЕЖЕДНЕВНЫЙ ОТЧЁТ'; title.font = { bold: true, size: 14, color: { argb: 'FFFFFFFF' } };
  title.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: BLUE } }; title.alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getRow(r).height = 28; r++;
  ws.mergeCells(r, 1, r, 2); label(ws, r, 1, 'Дата отчёта:'); ws.mergeCells(r, 3, r, 4); val(ws, r, 3, formatDateRu(report.report_date));
  ws.mergeCells(r, 5, r, 5); label(ws, r, 5, 'Неделя:'); ws.mergeCells(r, 6, r, 7); val(ws, r, 6, report.week_number);
  ws.getRow(r).height = 20; r++; r++;
  ws.mergeCells(r, 1, r, 7); h(ws, r, 1, 'ОБЩИЙ ОПЕРАЦИОННЫЙ КОНТРОЛЬ', BLUE); ws.getRow(r).height = 22; r++;
  ws.mergeCells(r, 2, r, 2); label(ws, r, 2, 'Точность данных:'); ws.mergeCells(r, 3, r, 7); statusCell(ws, r, 3, report.data_accuracy); ws.getRow(r).height = 30; r++;
  const accRows = report.accuracy_rows as IncidentRow[];
  if (accRows.some(row => row.responsible || row.details)) {
    label(ws, r, 2, 'Ответственный'); label(ws, r, 3, 'Детали'); label(ws, r, 4, 'Решение'); ws.getRow(r).height = 18; r++;
    for (const row of accRows) { if (row.responsible || row.details || row.resolution) { val(ws, r, 2, row.responsible); val(ws, r, 3, row.details, true); val(ws, r, 4, row.resolution, true); ws.getRow(r).height = 18; r++; } }
  }
  ws.mergeCells(r, 2, r, 2); label(ws, r, 2, 'Инциденты:'); ws.mergeCells(r, 3, r, 7); statusCell(ws, r, 3, report.incidents_status); ws.getRow(r).height = 22; r++;
  const incRows = report.incidents_rows as IncidentRow[];
  if (incRows.some(row => row.responsible || row.details)) {
    label(ws, r, 2, 'Ответственный'); label(ws, r, 3, 'Детали'); label(ws, r, 4, 'Решение'); ws.getRow(r).height = 18; r++;
    for (const row of incRows) { if (row.responsible || row.details || row.resolution) { val(ws, r, 2, row.responsible); val(ws, r, 3, row.details, true); val(ws, r, 4, row.resolution, true); ws.getRow(r).height = 18; r++; } }
  }
  r++;
  ws.mergeCells(r, 1, r, 7); h(ws, r, 1, 'CUSTOMER SERVICE', '9333EA'); ws.getRow(r).height = 22; r++;
  for (const [lbl, v, note] of [
    ['Всего заявок за день', report.cs_total, ''],
    ['Просрочено заявок', report.cs_overdue, report.cs_overdue_reason ? `Причина: ${report.cs_overdue_reason}` : ''],
    ['Средний срок выполнения (текущая неделя)', report.cs_avg_week, 'Цель: 2,5 дней'],
    ['Средний срок выполнения (текущий месяц)', report.cs_avg_month, 'Цель: 2,5 дней'],
  ] as [string, string | number, string][]) {
    ws.mergeCells(r, 2, r, 2); label(ws, r, 2, lbl); val(ws, r, 3, v); if (note) val(ws, r, 4, note); ws.getRow(r).height = 20; r++;
  }
  r++;
  function writeMetrics2(title: string, color: string, lightColor: string, prefix: 'uzum' | 'cainiao') {
    ws.mergeCells(r, 1, r, 7); h(ws, r, 1, title, color); ws.getRow(r).height = 22; r++;
    for (const [c, t] of [[2, 'Метрика'], [4, 'Всего'], [5, 'Шанхай'], [6, 'Гонконг']] as [number, string][]) {
      const cell = ws.getCell(r, c); cell.value = t; cell.font = { bold: true, size: 10, color: { argb: 'FF374151' } };
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: lightColor } }; cell.alignment = { horizontal: 'center', vertical: 'middle' };
    }
    ws.mergeCells(r, 2, r, 3); ws.getRow(r).height = 18; r++;
    const sh = `${prefix}_sh` as const; const hk = `${prefix}_hk` as const;
    const rows2: [string, number | string, number | string, number | string][] = [
      ['Кол-во принятых партий', (report[`${sh}_count`] as number) + (report[`${hk}_count`] as number), report[`${sh}_count`] as number, report[`${hk}_count`] as number],
      ['Общий вес партий, кг', (report[`${sh}_weight`] as number) + (report[`${hk}_weight`] as number), report[`${sh}_weight`] as number, report[`${hk}_weight`] as number],
      ['МКО, кг', (report[`${sh}_mko`] as number) + (report[`${hk}_mko`] as number), report[`${sh}_mko`] as number, report[`${hk}_mko`] as number],
      ['МПО, кг', (report[`${sh}_mpo`] as number) + (report[`${hk}_mpo`] as number), report[`${sh}_mpo`] as number, report[`${hk}_mpo`] as number],
    ...
    for (const [lbl2, total, shVal, hkVal] of rows2) { ws.mergeCells(r, 2, r, 3); label(ws, r, 2, lbl2); val(ws, r, 4, total); val(ws, r, 5, shVal); val(ws, r, 6, hkVal); ws.getRow(r).height = 18; r++; }
    r++;
    for (const [opLabel, statusKey, incidentKey] of [
      ['Операции в Китае', `${prefix}_china_status`, `${prefix}_china_incident`],
      ['Транзитные операции', `${prefix}_transit_status`, `${prefix}_transit_incident`],
      ...
      ws.mergeCells(r, 2, r, 3); label(ws, r, 2, opLabel); ws.mergeCells(r, 4, r, 7); statusCell(ws, r, 4, report[statusKey] as string); ws.getRow(r).height = 20; r++;
      const incident = report[incidentKey] as string;
      if (incident) { ws.mergeCells(r, 2, r, 3); label(ws, r, 2, 'Описание инцидента:'); ws.mergeCells(r, 4, r, 7); val(ws, r, 4, incident, true); ws.getRow(r).height = 36; r++; }
    }
    r++;
  }
  writeMetrics2('UZUM CROSSBORDER', ORANGE, LIGHT_ORANGE, 'uzum');
  writeMetrics2('CAINIAO', TEAL, LIGHT_TEAL, 'cainiao');
  border(ws, 1, r - 1, 1, 7);
}
